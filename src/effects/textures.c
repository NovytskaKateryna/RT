...
/*   Updated: 2018/06/11 23:06:48 by vvinogra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	*get_textures(SDL_Surface **surf, int len)
{
	...
	array = (unsigned int *)malloc(sizeof(unsigned int) * (len * 6));
	perlin = perlin_noise();
	check = checkerboard();
	...
	ft_memcpy(&array[len * 4], perlin, len);
	ft_memcpy(&array[len * 5], check, len);
	free(perlin);
	free(check);
	return (array);
}
